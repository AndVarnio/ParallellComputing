...
__host__ __device__ void runVertexShader( float4 &vertex,
...
__host__ __device__ void runFragmentShader( unsigned char* frameBuffer,
...
__global__ void renderMeshes(
		...
		if(threadIdx.x<totalItemsToRender){
			workItemGPU objectToRender = workQueue[threadIdx.x];
			// if(threadIdx.y<meshCount){
			// unsigned int meshIndex = threadIdx.y;
...
__global__ void initFrameBuffer(unsigned char* buffer, unsigned int width, unsigned int height){
	if(blockIdx.x*blockDim.x*blockDim.y*blockDim.z+threadIdx.z*blockDim.y*blockDim.x+threadIdx.y*blockDim.x+4*threadIdx.x+3<4*width*height){
		buffer[blockIdx.x*blockDim.x*blockDim.y*blockDim.z+threadIdx.z*blockDim.y*blockDim.x+threadIdx.y*blockDim.x+4*threadIdx.x + 0] = 0;
		buffer[blockIdx.x*blockDim.x*blockDim.y*blockDim.z+threadIdx.z*blockDim.y*blockDim.x+threadIdx.y*blockDim.x+4*threadIdx.x + 1] = 0;
		buffer[blockIdx.x*blockDim.x*blockDim.y*blockDim.z+threadIdx.z*blockDim.y*blockDim.x+threadIdx.y*blockDim.x+4*threadIdx.x + 2] = 0;
		buffer[blockIdx.x*blockDim.x*blockDim.y*blockDim.z+threadIdx.z*blockDim.y*blockDim.x+threadIdx.y*blockDim.x+4*threadIdx.x + 3] = 255;
	}
}

__global__ void initDepthBuffer(int* bufferDepth, int width, int height){
	if(blockIdx.x*blockDim.x*blockDim.y*blockDim.z+threadIdx.z*blockDim.x*blockDim.y+threadIdx.y*blockDim.x+threadIdx.x<4*width*height)
	bufferDepth[blockIdx.x*blockDim.x*blockDim.y*blockDim.z+threadIdx.z*blockDim.x*blockDim.y+threadIdx.y*blockDim.x+threadIdx.x] = 16777216;
}
...
std::vector<unsigned char> rasteriseGPU(std::string inputFile, unsigned int width, unsigned int height, unsigned int depthLimit) {
	...
	// }
		...
		cudaError_t(cudaMalloc((void **)&frameBufferPtr, width*height*4*sizeof(char)));

		int gridSize = (4*width*height + 512 - 1) / 512;
		dim3 blockSize(8, 8, 8);
		initFrameBuffer<<<gridSize, blockSize>>>(frameBufferPtr, width, height);
		...
		cudaError_t(cudaMalloc((void **)&depthBufferPtr, width*height*sizeof(int)));



		gridSize = (width*height + 512 - 1) / 512;

		initDepthBuffer<<<gridSize, blockSize>>>(depthBufferPtr, width, height);
		cudaDeviceSynchronize();

		//cudaError_t(cudaMemset(depthBufferPtr, 16777126, width*height*sizeof(int)));
    ...
    for(unsigned int i = 0; i < meshes.size(); i++) {
		...
		GPUMesh* meshesCpu = new GPUMesh[meshes.size()];

		float4** hostAdressesForVerticesOnDevice;
		float3** hostAdressesForNormalsOnDevice;

		hostAdressesForVerticesOnDevice = (float4**)malloc(meshes.size()*sizeof(float4*));
		hostAdressesForNormalsOnDevice = (float3**)malloc(meshes.size()*sizeof(float3*));
		for(unsigned int i=0; i<meshes.size(); i++){
			cudaError_t(cudaMalloc((void **)&hostAdressesForVerticesOnDevice[i], meshes.at(i).vertexCount*sizeof(float4)));
			cudaError_t(cudaMalloc((void **)&hostAdressesForNormalsOnDevice[i], meshes.at(i).vertexCount*sizeof(float3)));

			cudaError_t(cudaMemcpy(hostAdressesForVerticesOnDevice[i], meshes.at(i).vertices, meshes.at(i).vertexCount*sizeof(float4), cudaMemcpyHostToDevice));
			cudaError_t(cudaMemcpy(hostAdressesForNormalsOnDevice[i], meshes.at(i).normals, meshes.at(i).vertexCount*sizeof(float3), cudaMemcpyHostToDevice));

		}

		float4** deviceAdressesForVerticesOnDevice;
		float3** deviceAdressesForNormalsOnDevice;

		cudaError_t(cudaMalloc(&deviceAdressesForVerticesOnDevice, meshes.size()*sizeof(float4*)));
		cudaError_t(cudaMalloc(&deviceAdressesForNormalsOnDevice, meshes.size()*sizeof(float3*)));

		cudaMemcpy(deviceAdressesForVerticesOnDevice, hostAdressesForVerticesOnDevice, meshes.size()*sizeof(float4*), cudaMemcpyHostToDevice);
		cudaMemcpy(deviceAdressesForNormalsOnDevice, hostAdressesForNormalsOnDevice, meshes.size()*sizeof(float3*), cudaMemcpyHostToDevice);

		for(unsigned int i=0; i<meshes.size(); i++){
			meshesCpu[i].vertices = hostAdressesForVerticesOnDevice[i];
			meshesCpu[i].normals = hostAdressesForNormalsOnDevice[i];
		}

		for(unsigned int i=0; i<meshes.size(); i++){
			meshesCpu[i].vertexCount = meshes.at(i).vertexCount;
			meshesCpu[i].objectDiffuseColour = meshes.at(i).objectDiffuseColour;
			meshesCpu[i].hasNormals = meshes.at(i).hasNormals;
		}

		GPUMesh* meshesGpu;
		cudaError_t(cudaMalloc((void **)&meshesGpu, meshes.size()*sizeof(GPUMesh)));
		cudaMemcpy(meshesGpu, meshesCpu, meshes.size()*sizeof(GPUMesh), cudaMemcpyHostToDevice);
    ...
    fillWorkQueue(workQueue, largestBoundingBoxSide, depthLimit, &counter);
		...
		workItemGPU* workItemGPUPtr;
		cudaError_t(cudaMalloc((void **)&workItemGPUPtr, totalItemsToRender*sizeof(workItemGPU)));
		cudaError_t(cudaMemcpy(workItemGPUPtr, workQueue, totalItemsToRender*sizeof(workItemGPU), cudaMemcpyHostToDevice));

		blockSize = {(unsigned int)totalItemsToRender, (unsigned int)meshes.size()};
		renderMeshes<<<1, totalItemsToRender>>>(totalItemsToRender, workItemGPUPtr,
		// renderMeshes<<<1, totalItemsToRender>>>(totalItemsToRender, workItemGPUPtr,
			meshesGpu, meshes.size(),
			width, height, frameBufferPtr, depthBufferPtr);
			cudaDeviceSynchronize();
		// renderMeshes(
    ...
    std::cout << "Finished!" << std::endl;
		...
		cudaError_t(cudaMemcpy(frameBuffer, frameBufferPtr, width*height*4*sizeof(unsigned char), cudaMemcpyDeviceToHost));
		cudaDeviceSynchronize();
