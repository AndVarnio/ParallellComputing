...
int main( int argc, char *argv[] )
{
	...
	for (unsigned int y = 0; y < res; y++) {
		#pragma omp parallel for num_threads(thread_count)// reduction(: pixel)
		for (unsigned int x = 0; x < res; x++) {
			...
			*(pixel+(y*4*res+4*x)) = colour.r;
			*(pixel+(y*4*res+4*x+1)) = colour.g;
			*(pixel+(y*4*res+4*x+2)) = colour.b;
			*(pixel+(y*4*res+4*x+3)) = colour.a;
		}
	}
